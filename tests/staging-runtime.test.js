...
function request(server, path, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    if (options.body !== undefined) { req.setHeader('content-type', 'application/json'); req.write(JSON.stringify(options.body)); }
    req.end();
  });
}
function json(response) { return response.body ? JSON.parse(response.body) : {}; }
function cookieFrom(response, current = '') { return response.headers['set-cookie']?.[0]?.split(';', 1)[0] || current; }
...
async function withServer(work) {
  ...
  try { return await work(server); }
  finally {
    server.close();
    await once(server, 'close');
    if (originalNodeEnv === undefined) delete process.env.NODE_ENV; else process.env.NODE_ENV = originalNodeEnv;
    if (originalAdminEmail === undefined) delete process.env.INBOX9_LOCAL_ADMIN_EMAIL; else process.env.INBOX9_LOCAL_ADMIN_EMAIL = originalAdminEmail;
    resetMocks();
  }
}
...
await withServer(async (server) => {
  ...
  const register = await request(server, '/api/auth/register', { method: 'POST', body: { email, password } });
  assert.equal(register.status, 201);
  assert.equal(json(register).user.email, email);
  let cookie = cookieFrom(register); assert.ok(cookie);
  const me = await request(server, '/api/auth/me', { headers: { cookie } });
  assert.equal(me.status, 200); assert.equal(json(me).authenticated, true);
  const userCookie = cookie;
  const recharge = await request(server, '/api/recharges', { method: 'POST', headers: { cookie, origin: 'http://127.0.0.1' }, body: { amount: 100, utr: 'RUNTIME-UTR-0001' } });
  assert.equal(recharge.status, 201);
  const adminRegister = await request(server, '/api/auth/register', { method: 'POST', body: { email: 'admin-runtime@example.test', password: 'RuntimeAdmin!123' } });
  ...
  assert.ok(pendingRecharge);
  const approval = await request(server, '/api/admin/recharges/' + encodeURIComponent(pendingRecharge.id), { method: 'POST', headers: { cookie: adminCookie, origin: 'http://127.0.0.1' }, body: { decision: 'approve' } });
  assert.equal(approval.status, 200);
  cookie = userCookie;
  const wallet = await request(server, '/api/wallet', { headers: { cookie } });
  assert.equal(wallet.status, 200); assert.equal(json(wallet).balancePaise, 10000);
  const activation = await request(server, '/api/activations', { method: 'POST', headers: { cookie, 'idempotency-key': 'runtime-activation-0001', origin: 'http://127.0.0.1' }, body: { serviceId: catalog[0].id } });
  assert.equal(activation.status, 201);
  const activationBody = json(activation);
  assert.equal(activationBody.serviceId, catalog[0].id);
  ...
  assert.equal((await request(server, '/api/auth/me', { headers: { cookie } })).status, 401);
});
