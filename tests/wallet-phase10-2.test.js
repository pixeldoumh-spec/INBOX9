...
const wallet = fs.readFileSync(
  path.join(root, 'frontend/src/app/customer-account-pages.tsx'),
  'utf8',
);
const css = fs.readFileSync(
  path.join(root, 'frontend/src/styles/customer-modern.css'),
  'utf8',
);
...
test('phase 10.2 wallet refreshes after reconnect and browser focus', () => {
  assert.match(
    wallet,
    /queryKey:\['wallet'\],queryFn:getWallet,refetchInterval:10000,refetchOnReconnect:true,refetchOnWindowFocus:true/,
  );
...
test('phase 10.2 never claims UPI payment time when only opening the UPI app', () => {
  ...
  assert.match(
    wallet,
    /function markPaymentCompleted\(\)\{openPaymentConfirm\(\);if\(!customerPaidAt\)setCustomerPaidAt\(localDateTimeNow\(\)\)\}/,
  );
  assert.match(
    wallet,
    /function openUpi\(\)\{if\(!upiIntent\|\|!online\)return;openPaymentConfirm\(\);window\.location\.href=upiIntent\}/,
  );
...
test('phase 10.2 pauses recharge submission while offline and exposes the reason', () => {
  assert.match(
    wallet,
    /if\(!online\)\{setError\("Reconnect before submitting the recharge\."\);return\}/,
  );
...
test('phase 10.2 payment-time validation remains bounded and server-side', () => {
  ...
  assert.throws(
    () => normalizeCustomerPaidAt('not-a-date'),
    /Enter a valid payment date and time/,
  );
  assert.throws(
    () => normalizeCustomerPaidAt(new Date(Date.now() + 6 * 60 * 1000).toISOString()),
    /Payment time cannot be in the future/,
  );
  assert.throws(
    () => normalizeCustomerPaidAt(new Date(Date.now() - 31 * 24 * 60 * 60 * 1000).toISOString()),
    /Payment time must be within the last 30 days/,
  );
});

test('phase 10.2 keeps the server credit gate after manual approval', () => {
  const repo = fs.readFileSync(
    path.join(root, 'api/_lib/wallet-repository.js'),
    'utf8',
  );
  assert.match(repo, /decision\s*===\s*['"]approve['"]/);
  assert.match(repo, /normalized\.amountPaise\s*==\s*null/);
  assert.match(repo, /normalized\.utr\s*==\s*null/);
  assert.match(
    repo,
