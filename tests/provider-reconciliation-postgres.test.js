...
test('db and supabase migration trees stay semantically aligned', async () => {
  ...
  const supabaseNames = supabaseFiles.filter(name => /^\d{12}_.+\.sql$/.test(name)).map(migrationSemanticName).sort();
...
test('persistent expiration claim and finalization execute against PostgreSQL', async () => {
  if (!testDb()) return;
  process.env.DATABASE_URL = testDb();
  process.env.DATABASE_SSL = 'false';

  const { getPool } = await import('../api/_lib/db.js');
  const { reconcileExpiringActivations } = await import('../api/_lib/provider-operations.js');
  const pool = await getPool();
  const suffix = crypto.randomUUID();
  ...
  try {
    ...
    const results = await reconcileExpiringActivations({ limit: 1 });
    assert.equal(results.length, 1);
    assert.equal(results[0]?.status, 'Expired');

    const [activation, stock] = await Promise.all([
      pool.query('SELECT status FROM activations WHERE id=$1', [activationId]),
      pool.query('SELECT stock FROM services WHERE id=$1', [serviceId])
    ]);
    assert.equal(activation.rows[0]?.status, 'Expired');
    assert.equal(Number(stock.rows[0]?.stock), 2);
  } finally {
    await pool.query('DELETE FROM provider_operations WHERE activation_id=$1', [activationId]).catch(() => {});
    await pool.query('DELETE FROM activations WHERE id=$1', [activationId]).catch(() => {});
    await pool.query('DELETE FROM services WHERE id=$1', [serviceId]).catch(() => {});
