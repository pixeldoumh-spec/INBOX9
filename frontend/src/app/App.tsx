...
function resolvePostLoginPath(next:string|null,role?:string){const fallback=role==='admin'?'/admin':'/apps';const candidate=String(next||'').trim();if(!candidate.startsWith('/')||candidate.startsWith('//'))return fallback;if((candidate==='/admin'||candidate.startsWith('/admin/'))&&role!=='admin')return '/apps';return candidate;}
