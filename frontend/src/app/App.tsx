...
function AdminPaymentsPage(){
 ...
 const pending=pendingQ.data?.recharges??[];const flagged=reconQ.data?.flagged??[];const manualEvents=reconQ.data?.manualEvents??[];const events=reconQ.data?.webhookEvents??[];const summary=reconQ.data?.summary;const configReady=Boolean(settings.upiId)&&Boolean(settings.instructions.trim());const manualMode=settings.enabled===true?'Enabled':settings.enabled===false?'Disabled':'Deployment controlled';
 ...
 return <section className="page-section admin-payments-page"><div className="catalog-heading"><div><span className="catalog-eyebrow">ADMIN · MANUAL PAYMENTS</span><h1>Manual payment operations</h1><p>Review manual UPI recharges, maintain the payment destination, and keep the full verification trail.</p></div><span className="catalog-chip">{pending.length} pending</span></div>
  <div className="admin-payment-summary"><div><span>Manual mode</span><strong>{manualMode}</strong><small>{settings.enabled==null?'Deployment setting':'Admin controlled'}</small></div><div><span>Total requests</span><strong>{summary?.totals.count??0}</strong><small>All recharge requests</small></div><div><span>Submitted value</span><strong>₹{((summary?.totals.amountPaise??0)/100).toFixed(2)}</strong><small>Total requested value</small></div><div><span>Flagged</span><strong>{summary?.totals.flaggedCount??0}</strong><small>Manual investigation signals</small></div></div>
  ...
  <div className="admin-payment-panel admin-payment-history-panel"><div className="section-heading"><div> <span className="catalog-eyebrow">MANUAL REVIEW HISTORY</span><h2>Payment events & flags</h2><span className="section-subtle">Investigate exceptions without directly editing wallet balances.</span></div></div>{manualEvents.length?<div className="admin-signal-list">{manualEvents.slice(0,30).map(event=><div className="admin-signal-row" key={'m-'+event.id}><div><strong>{({submitted:'Submitted',verified:'Verified',approved:'Approved',rejected:'Rejected',flagged:'Flagged'} as Record<string,string>)[event.eventType]||event.eventType} · {event.customerEmail}</strong><span>₹{(event.amountPaise/100).toFixed(2)} · {event.rechargeStatus}</span><span>{event.utr?'UTR · '+event.utr:'No UTR'} · {new Date(event.createdAt).toLocaleString()}</span>{event.notes?<span>{event.notes}</span>:null}</div><span>{event.actorEmail||'System'}</span></div>)}</div>:<div className="empty-state compact-empty"><h3>No manual payment events</h3><p>The manual review trail will appear here as customers submit and admins review recharges.</p></div>}{flagged.length?<div className="admin-signal-list">{flagged.slice(0,20).map(r=><div className="admin-signal-row" key={'f-'+r.id}><div><strong>{r.email}</strong><span>₹{(r.amountPaise/100).toFixed(2)} · {r.status}</span><span>{r.flagReason||'Flagged for review'}</span></div><span>{r.utr}</span></div>)}</div>:<div className="empty-state compact-empty"><h3>No flagged requests</h3><p>Requests that need investigation will appear here.</p></div>}{events.length?<div className="admin-signal-list admin-webhook-list">{events.slice(0,20).map(event=><div className="admin-signal-row" key={event.provider+'-'+event.eventId}><div><strong>{event.eventType}</strong><span>{event.provider} · {event.outcome||event.status}</span><span>{event.rechargeId}</span></div><span>{event.amountPaise?`₹${(event.amountPaise/100).toFixed(2)}`:'—'}</span></div>)}</div>:<div className="empty-state compact-empty"><h3>No webhook events</h3><p>Verified gateway events will appear here when an automated gateway is connected.</p></div>}</div>
 </section>
}
