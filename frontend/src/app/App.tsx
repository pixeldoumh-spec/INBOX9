...
function AppShell(){const bootstrap=useSessionStore(s=>s.bootstrap);const user=useSessionStore(s=>s.user);const location=useLocation();if(bootstrap==='idle'||bootstrap==='loading')return <div className="splash"><div className="splash-mark">I9</div><span>Loading INBOX9...</span></div>;if(!user)return <Navigate to={`/login?next=${encodeURIComponent(location.pathname+location.search)}`} replace/>;return <div className="app-shell"><TopHeader/><main className="page-content"><Outlet/></main><BottomNav/></div>}
...
function SupportPage(){return <section className="page-section"><Link className="back-link" to="/account"><Icon name="back" size={18}/> Account</Link><div className="catalog-heading"><div><h1>Support</h1><p>Need help? Send a request and keep your account in one place.</p></div></div><div className="info-card"><Icon name="support" size={20}/><div><strong>Support is connected to the INBOX9 backend.</strong><p>Ticket creation and history can be enabled here without changing the app shell.</p></div></div></section>}
