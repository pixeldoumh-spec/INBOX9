...
function AdminPaymentsPage(){
 ...
 const pendingQ=useQuery({queryKey:['admin-recharges'],queryFn:getAdminRecharges,refetchInterval:10000});
 const reconQ=useQuery({queryKey:['admin-payment-reconciliation'],queryFn:getAdminPaymentReconciliation,refetchInterval:15000});
 const [ledgerQuery,setLedgerQuery]=useState('');
 const [ledgerType,setLedgerType]=useState<'all'|'credit'|'debit'>('all');
 const [ledgerOffset,setLedgerOffset]=useState(0);
 const ledgerPageSize=30;
 const ledgerQ=useQuery({queryKey:['admin-ledger',ledgerQuery,ledgerType,ledgerOffset],queryFn:()=>getAdminLedger({q:ledgerQuery,type:ledgerType,limit:ledgerPageSize,offset:ledgerOffset}),refetchInterval:15000});
 const ledger=Array.isArray(ledgerQ.data?.ledger)?ledgerQ.data.ledger:[];
 const ledgerSummary=ledgerQ.data?.summary??{total:0,credits:0,debits:0,creditPaise:0,debitPaise:0};
 const ledgerHasMore=ledgerQ.data?.pagination?.hasMore??false;
 const client=useQueryClient();
 const [selectedId,setSelectedId]=useState<string|null>(null);
 const [decision,setDecision]=useState<'approve'|'reject'|'flag'>('approve');
 const [verifiedAmount,setVerifiedAmount]=useState('');
 const [verifiedUtr,setVerifiedUtr]=useState('');
 const [externalReference,setExternalReference]=useState('');
 const [reason,setReason]=useState('');
 const [settings,setSettings]=useState({enabled:null as boolean|null,upiId:'',merchantName:'INBOX9',instructions:'Pay the exact amount, then submit the UTR for verification.',qrImage:''});
 const [settingsMsg,setSettingsMsg]=useState<string|null>(null);
 const [settingsError,setSettingsError]=useState<string|null>(null);
 const [reviewError,setReviewError]=useState<string|null>(null);
 useEffect(()=>{const s=reconQ.data?.paymentSettings;if(s)setSettings({enabled:s.enabled??null,upiId:s.upiId||'',merchantName:s.merchantName||'INBOX9',instructions:s.instructions||'Pay the exact amount, then submit the UTR for verification.',qrImage:s.qrImage||''})},[reconQ.data?.paymentSettings]);
 ...
 const saveSettings=useMutation({mutationFn:()=>updatePaymentSettings(settings),onSuccess:async r=>{if(r.paymentSettings)setSettings({enabled:r.paymentSettings.enabled??null,upiId:r.paymentSettings.upiId||'',merchantName:r.paymentSettings.merchantName,instructions:r.paymentSettings.instructions,qrImage:r.paymentSettings.qrImage||''});setSettingsMsg('Manual payment settings saved.');setSettingsError(null);await reconQ.refetch()},onError:e=>{setSettingsMsg(null);setSettingsError(e instanceof Error?e.message:'Manual payment settings update failed')}});
 if(!user||user.role!=='admin')return <section className="page-section"><div className="error-card">Admin access required.</div></section>;
 const pending=pendingQ.data?.recharges??[];
 const flagged=reconQ.data?.flagged??[];
 const manualEvents=reconQ.data?.manualEvents??[];
 const summary=reconQ.data?.summary;
 const manualMode=settings.enabled===true?'Enabled':settings.enabled===false?'Disabled':'Deployment controlled';
 const configReady=Boolean(settings.upiId)&&Boolean(settings.instructions.trim());
 ...
 const manualEventLabel=(value:string)=>({submitted:'Submitted',verified:'Verified',approved:'Approved',rejected:'Rejected',flagged:'Flagged'} as Record<string,string>)[value]||value;
 return <section className="page-section admin-payments-page">
  <div className="catalog-heading"><div><span className="catalog-eyebrow">ADMIN · MANUAL PAYMENTS</span><h1>Manual payment operations</h1><p>Operate UPI recharges manually: configure the destination, verify customer payments, credit wallets, and keep the full audit trail.</p></div><span className="catalog-chip">{pending.length} pending</span></div>
  <div className="admin-payment-summary">
   <div><span>Manual mode</span><strong>{manualMode}</strong><small>{settings.enabled==null?'Still using the deployment recharge setting':'Controlled from this admin panel'}</small></div>
   <div><span>Total requests</span><strong>{summary?.totals.count??0}</strong><small>All manual recharge requests</small></div>
   <div><span>Submitted value</span><strong>₹{((summary?.totals.amountPaise??0)/100).toFixed(2)}</strong><small>Total requested value</small></div>
   <div><span>Flagged</span><strong>{summary?.totals.flaggedCount??0}</strong><small>Manual investigation signals</small></div>
  </div>
  <div className="admin-payment-panel">
   <div className="section-heading"><div><span className="catalog-eyebrow">SETUP</span><h2>Manual payment destination</h2><span className="section-subtle">Customer Wallet uses these values directly for manual UPI recharge.</span></div>
    <button className={settings.enabled===true?'secondary-danger':'primary-button'} type="button" disabled={saveSettings.isPending||(settings.enabled!==true&&!configReady)} onClick={()=>{setSettingsMsg(null);setSettingsError(null);void saveSettings.mutate()}}>{saveSettings.isPending?'Saving…':settings.enabled===true?'Disable manual recharge':'Enable manual recharge'}</button>
   </div>
   <div className="admin-payment-summary">
    <div><span>UPI destination</span><strong>{settings.upiId?'Configured':'Missing'}</strong><small>Customer payment address</small></div>
    <div><span>QR</span><strong>{settings.qrImage?'Configured':'Optional'}</strong><small>Customer scan option</small></div>
    <div><span>Instructions</span><strong>{settings.instructions.trim()?'Configured':'Missing'}</strong><small>Customer payment guidance</small></div>
    <div><span>Readiness</span><strong>{configReady?'Ready':'Needs setup'}</strong><small>{configReady?'Manual recharge can be enabled':'Set UPI ID and instructions first'}</small></div>
   </div>
   <form className="form-stack" onSubmit={e=>{e.preventDefault();setSettingsMsg(null);setSettingsError(null);void saveSettings.mutate()}}>
    <label className="field"><span>Merchant UPI ID</span><input value={settings.upiId} onChange={e=>setSettings(s=>({...s,upiId:e.target.value}))} placeholder="merchant@upi" required/></label>
    <label className="field"><span>Merchant name</span><input value={settings.merchantName} onChange={e=>setSettings(s=>({...s,merchantName:e.target.value}))} maxLength={80}/></label>
    <label className="field"><span>Customer instructions</span><textarea value={settings.instructions} onChange={e=>setSettings(s=>({...s,instructions:e.target.value}))} maxLength={500}/></label>
    <label className="field"><span>QR image URL or image data</span><textarea value={settings.qrImage} onChange={e=>setSettings(s=>({...s,qrImage:e.target.value}))} maxLength={350000} placeholder="https://.../qr.png or image data URI"/></label>
    {settingsError?<div className="form-error" role="alert">{settingsError}</div>:null}
    {settingsMsg?<div className="success-card" role="status"><Icon name="check" size={17}/><span>{settingsMsg}</span></div>:null}
    <button type="submit" className="primary-button" disabled={saveSettings.isPending}>{saveSettings.isPending?'Saving…':'Save payment settings'}<Icon name="check" size={16}/></button>
   </form>
   <div className="admin-payment-note"><Icon name="wallet" size={16}/><span>No automatic gateway or webhook is required. Customer wallet balance changes only through this server-side manual review path.</span></div>
  </div>
  <div className="admin-payment-grid">
   <div className="admin-payment-panel"><div className="section-heading"><div><span className="catalog-eyebrow">QUEUE</span><h2>Pending recharge requests</h2><span className="section-subtle">Verify exact amount + UTR before crediting.</span></div><button className="text-button compact-button" type="button" onClick={()=>void pendingQ.refetch()} disabled={pendingQ.isFetching}>Refresh</button></div>
    ...
    {pending.length?<div className="admin-recharge-list">{pending.map(r=><div className={'admin-recharge-row'+(selectedId===r.id?' is-selected':'')} key={r.id}>
      ...
      {selectedId===r.id?<div className="admin-review-form"><div className="admin-review-switch"><button type="button" className={decision==='approve'?'is-selected':''} onClick={()=>setDecision('approve')}>Approve</button><button type="button" className={decision==='flag'?'is-selected':''} onClick={()=>setDecision('flag')}>Flag</button><button type="button" className={decision==='reject'?'is-selected':''} onClick={()=>setDecision('reject')}>Reject</button></div>
       {decision!=='reject'?<><label className="field"><span>Verified amount</span><div className="money-input"><span>₹</span><input value={verifiedAmount} onChange={e=>setVerifiedAmount(e.target.value)} type="number" min="100" max="5000" step="1" required={decision==='approve'}/></div></label><label className="field"><span>Verified UTR</span><input value={verifiedUtr} onChange={e=>setVerifiedUtr(e.target.value)} maxLength={64} required={decision==='approve'}/></label><label className="field"><span>Bank/reference note <small>(optional)</small></span><input value={externalReference} onChange={e=>setExternalReference(e.target.value)} maxLength={120} placeholder="Statement or counterparty reference"/></label></>:null}
       <label className="field"><span>{decision==='approve'?'Review note':decision==='flag'?'Flag reason':'Rejection reason'}</span><textarea value={reason} onChange={e=>setReason(e.target.value)} maxLength={500} placeholder={decision==='approve'?'Optional internal note':'Explain what was verified or why this needs review'} required={decision!=='approve'}/></label>
       {reviewError?<div className="form-error" role="alert">{reviewError}</div>:null}
       <div className="admin-review-actions"><button type="button" className={decision==='reject'?'secondary-danger':'primary-button'} disabled={review.isPending} onClick={()=>void review.mutate()}>{review.isPending?'Saving…':decision==='approve'?'Approve & credit':decision==='flag'?'Flag for review':'Reject recharge'}<Icon name={decision==='reject'?'close':'check'} size={16}/></button><button type="button" className="text-button" onClick={()=>setSelectedId(null)} disabled={review.isPending}>Cancel</button></div>
      </div>:null}
     </div>)}</div>:<div className="empty-state compact-empty"><h3>No pending recharges</h3><p>New manual UPI requests will appear here.</p></div>}
   </div>
   <div className="admin-payment-panel"><div className="section-heading"><div><span className="catalog-eyebrow">MANUAL REVIEW HISTORY</span><h2>Payment events</h2><span className="section-subtle">Submitted, verified, approved, rejected and flagged events.</span></div><button className="text-button compact-button" type="button" onClick={()=>void reconQ.refetch()} disabled={reconQ.isFetching}>Refresh</button></div>
    {reconQ.isError?<div className="error-card">Payment history unavailable. Retry the request.</div>:manualEvents.length?<div className="admin-signal-list">{manualEvents.slice(0,30).map(event=><div className="admin-signal-row" key={event.id}><div><strong>{manualEventLabel(event.eventType)} · {event.customerEmail}</strong><span>₹{(event.amountPaise/100).toFixed(2)} · {event.rechargeStatus}</span><span>{event.utr?'UTR · '+event.utr:'No observed UTR'} · {new Date(event.createdAt).toLocaleString()}</span>{event.notes?<span>{event.notes}</span>:null}</div><span>{event.actorEmail||'System'}</span></div>)}</div>:<div className="empty-state compact-empty"><h3>No manual payment events</h3><p>The audit trail will appear here as customers submit and admins review recharges.</p></div>}
   </div>
  </div>
