...
export type AdminReconciliationRun={id:string;trigger:'cron'|'manual';status:'Running'|'Succeeded'|'Failed';started_at:string;completed_at:string|null;cancellation_processed:number;expiration_processed:number;wallet_reconciliation_processed:number;error_code:string|null;error_message:string|null};
export type AdminReconciliationEvent={id:string;run_id:string;operation_id:string|null;activation_id:string|null;provider_id:string|null;event_type:string;outcome:string;safe_to_retry:boolean;error_code:string|null;error_message:string|null;details:Record<string,unknown>;created_at:string};
export type AdminReconciliationBacklog={id:string;activation_id:string;operation_type:string;status:string;provider_id:string|null;provider_activation_id:string|null;attempts:number;last_error:string|null;created_at:string;updated_at:string;completed_at:string|null;user_id:string;service_id:string;service_name:string;activation_status:string;expires_at:string;adapter_key:string|null};
export type AdminProvidersResponse={providers:AdminProvider[];health:AdminProviderHealth[];installedAdapters:string[];gateway:{version:number;timeoutsMs:Record<string,number>;metrics:string;safeReserveRetry:string;externalRoutingEnabled:boolean};routeHealth:AdminRouteHealth[];routeAttempts:AdminRouteAttempt[];reconciliation:{runs:AdminReconciliationRun[];events:AdminReconciliationEvent[];unresolvedBacklog:AdminReconciliationBacklog[]}};
