...
export function AdminDashboardPage(){
  ...
  return <section className="admin-page">
    ...
    <div className="admin-dashboard-grid">
      ...
      <section className="admin-panel"><div className="admin-panel-heading"><div><span className="admin-eyebrow">NEXT</span><h2>Operations modules</h2></div></div><div className="admin-module-list"><Link className="admin-module-link" to="/admin/users"><strong>Users</strong><span>User/account operations</span></Link><Link className="admin-module-link" to="/admin/services"><strong>Services</strong><span>Catalog and routing controls</span></Link><Link className="admin-module-link" to="/admin/activations"><strong>Activations</strong><span>Lifecycle monitoring and controlled cancellation</span></Link><div><strong>Providers</strong><span>Health and operations</span></div><div><strong>Reconciliation</strong><span>Wallet/payment integrity</span></div><Link className="admin-module-link" to="/admin/support"><strong>Support</strong><span>Customer tickets and threaded replies</span></Link><Link className="admin-module-link" to="/admin/notifications"><strong>Notifications</strong><span>Persistent delivery and targeted messages</span></Link><Link className="admin-module-link" to="/admin/reconciliation"><strong>Reconciliation & audit</strong><span>Wallet integrity and administrative history</span></Link><Link className="admin-module-link" to="/admin/health"><strong>System health</strong><span>Runtime, dependencies and operational signals</span></Link></div><small className="admin-module-note">These operational screens remain behind this isolated shell and are implemented in the subsequent Phase 11 stages.</small></section>
    </div>
