...
export function AdminUsersPage(){
  ...
  const list=useQuery({queryKey:['admin-users',q,role,status],queryFn:()=>getAdminUsers({q,role,status,limit:150}),staleTime:5_000,refetchInterval:15_000,refetchOnReconnect:true});
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>setQ(input.trim()),250);return()=>window.clearTimeout(timer)},[input]);
  ...
  return <section className="admin-page admin-users-page">
    ...
    <div className="admin-users-toolbar"><input className="admin-search-input" value={input} onChange={e=>setInput(e.target.value)} placeholder="Search email, name or user ID" aria-label="Search users"/><select className="admin-filter-select" value={role} onChange={e=>setRole(e.target.value)} aria-label="Filter by role"><option value="all">All roles</option><option value="user">Customers</option><option value="admin">Admins</option></select><select className="admin-filter-select" value={status} onChange={e=>setStatus(e.target.value)} aria-label="Filter by account status"><option value="all">All status</option><option value="active">Active</option><option value="disabled">Disabled</option></select></div>
    ...
    <div className="admin-users-layout">
      <section className="admin-panel admin-user-list-panel"><div className="admin-panel-heading"><div><span className="admin-eyebrow">DIRECTORY</span><h2>Accounts</h2></div><span className="admin-status-badge">{list.data?.users.length??0}</span></div>{list.isPending?<div className="admin-user-list">{Array.from({length:7},(_,i)=><div className="admin-user-skeleton" key={i}/>)}</div>:list.data?.users.length?<div className="admin-user-list">{list.data.users.map(u=><Link key={u.id} to={'/admin/users/'+encodeURIComponent(u.id)} className={'admin-user-row'+(u.id===userId?' is-selected':'')}><div className="admin-user-avatar">{(u.displayName||u.email).slice(0,1).toUpperCase()}</div><div className="admin-user-main"><strong>{u.displayName||u.email}</strong><span>{u.email}</span><small>{u.id}</small></div><div className="admin-user-side"><span className={statusClass(u.active)}>{u.active?'Active':'Disabled'}</span><strong>{money(u.balancePaise)}</strong><small>{u.activationCount} activations · {u.rechargeCount} recharges</small></div></Link>)}</div>:<div className="empty-state compact-empty"><h3>No users found</h3><p>Adjust the search or filters.</p></div>}</section>
      <section className="admin-panel admin-user-detail-panel">
        {!userId?<div className="admin-user-detail-empty"><div className="admin-detail-mark">U</div><h2>Select an account</h2><p>Choose a user from the directory to inspect account activity and session state.</p></div>:detail.isPending?<div className="admin-user-detail-empty"><div className="admin-detail-mark">…</div><h2>Loading account</h2><p>Fetching the latest account state.</p></div>:detail.isError?<div className="admin-user-detail-empty"><div className="admin-detail-mark">!</div><h2>User unavailable</h2><p>The account could not be loaded.</p><Link className="outline-button" to="/admin/users">Back to users</Link></div>:selected?<div className="admin-user-detail">
          ...
          <div className="admin-detail-meta"><span className={statusClass(selected.user.active)}>{selected.user.active?'Active':'Disabled'}</span><span>{selected.user.role==='admin'?'Admin':'Customer'}</span><span>Joined {when(selected.user.createdAt)}</span></div>
