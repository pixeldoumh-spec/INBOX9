...
function buyPage() {
  ...
  return `<div class="section-head"><div><span class="kicker">SANDBOX / DEMO</span><h2>Choose a service</h2></div><span class="result-note">${list.length} services</span></div>
    <div class="sandbox-notice" role="note"><strong>Sandbox demo</strong><span>Numbers and verification codes are simulated for testing only. They will not receive SMS or verify accounts on third-party services.</span></div>
...
function activeCard(activation) {
  ...
  return `<article class="active-card ${completed ? 'completed' : ''}><div class="active-card-header"><div class="service-icon large">${iconFor(state.services.find((s) => s.id === activation.serviceId)?.category)}</div><div class="service-meta"><span class="service-category">${esc(activation.service)} · Sandbox</span><h3>${esc(activation.number)}</h3></div><span class="activation-status">${completed ? '✓ Demo code ready' : '↻ Generating demo code'}</span></div>${completed ? `<div class="otp-panel"><div class="otp-label">DEMO VERIFICATION CODE</div><div class="otp-code">${esc(activation.otp)}</div><button class="copy-btn" type="button" data-copy="${esc(activation.otp.replace(/\s/g, ''))}">⧉ Copy OTP</button></div>` : `<div class="otp-panel"><div class="otp-label">TIME REMAINING</div><div class="timer">◷ ${minutes}:${seconds}</div><div class="progress"><span style="width:${progress}%"></span></div><div class="waiting-note">▣ Your verification code will appear here automatically</div></div>`}<div class="active-footer"><span>Order <strong class="mono">${esc(activation.id)}</strong></span><span>${money(activation.pricePaise)}</span>${!completed ? `<button class="text-danger" type="button" data-cancel="${esc(activation.id)}">Cancel & refund</button>` : ''}</div></article>`;
}
