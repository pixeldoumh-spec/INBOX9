...
import { getPaymentReconciliationSummary, listFlaggedRecharges, listPendingRecharges } from '../_lib/wallet-repository.js';
...
export default async function handler(req, res) {
  ...
  try {
    const [summary, flagged, webhookEvents, paymentSettings, sessionSignalsRaw] = await Promise.all([
    ...
    return res.status(200).json({ summary, flagged, webhookEvents, paymentSettings, sessionSignals, generatedAt: Date.now() });
