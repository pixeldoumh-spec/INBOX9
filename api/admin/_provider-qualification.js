...
import { qualifyProviders, verifyAndSaveProviderMapping } from '../_lib/provider-qualification.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const status = Number(error?.statusCode) || (error?.code === 'PROVIDER_NOT_CONFIGURED' ? 503 : error?.code === 'PROVIDER_SERVICE_CODE_NOT_FOUND' ? 400 : 500);
