...
const result = { ok: missing.length === 0 && serviceCount === 832 && activeRouteCount === 832 && orphanActivations === 0 && invalidChecks === 0 && unvalidatedRequiredFks === 0 && ledgerMismatch === 0 && migrationCount >= 27, requiredTables: { expected: required.length, found: found.size, missing }, rowCounts, invariants: { services: serviceCount, activeProviderRoutes: activeRouteCount, orphanActivations, activationsUserConstraintUnvalidated: invalidChecks, unvalidatedForeignKeys: unvalidatedRequiredFks, walletLedgerMismatches: ledgerMismatch, schemaMigrationCount: migrationCount, latestMigration: schemaVersion } };
